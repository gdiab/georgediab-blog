...
- **NEVER CREATE BLOG POSTS WITHOUT EXPLICIT CONSENT** from George
...
5. **Fixing or adding images**:

   - Copy new image assets into `/public/` (organize by purpose, e.g. `/public/posts/<slug>/`)
   - Reference them from markdown with relative `/posts/...` paths
   - Commit any added media alongside the post that uses it for easy tracking
